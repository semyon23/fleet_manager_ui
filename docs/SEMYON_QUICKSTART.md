...
В правом верхнем углу увидишь маленький кружок:
- **зелёный** + миллисекунды рядом — бэк ответил, всё ок
- **жёлтый** — бэк отвечает медленно (>2 сек)
- **красный** — 3 раза подряд не ответил, `offline`
- **пульсирующий оранжевый** — включены `mocks` (значит `.env.local` не подхватился)

Наведи мышку на кружок — увидишь подробности в тултипе.
...
## Как менять адрес бэка БЕЗ пересборки (runtime-конфиг)

Если ты не хочешь каждый раз тыкать `npm run build` при смене адреса/порта твоего бэка — есть файл `config.js`, который читается при загрузке страницы.

Работает в двух местах:
1. **В dev-режиме** — файл лежит в `public/config.js` в исходниках. Пустые значения = использовать `.env.local` (обычный сценарий).
2. **В собранной статике** — после `npm run build` файл появляется в `dist/config.js`. Открывай блокнотом, правь, обновляй страницу (F5) — пересборка не нужна.

Пример редактирования `dist/config.js`:

```js
window.__FLEET_CONFIG__ = {
  apiBaseUrl: 'http://192.168.0.111:5000/api',
  useMocks: false,
};
```

Приоритет полей:
1. Значение из `config.js` (если не пустая строка / не `null`)
2. `.env.local` (использовалось при сборке)
3. Дефолты кода

Если оставить `apiBaseUrl: ''` и `useMocks: null` — работает как раньше, всё берётся из сборки.

Этот механизм особенно удобен когда ты берёшь готовый `dist/` (например артефакт из GitHub Actions или мою сборку) и хочешь просто указать свой адрес бэка не пересобирая проект.
